composer require phpmailer/phpmailer bu komutu nasıl kullanacam bilmiyorum 
Register.php'yi Güncel halini atar mısn
şuan Register.php bende aşağıda ki gibi 
<?php
session_start();
    ...
    if (empty($username) || empty($email) || empty($password)) {
        ...
        try {
            ...
            if ($count > 0) {
                ...
                // Kullanıcıyı kaydet
                $stmt = $db->prepare("INSERT INTO users (username, email, password, folder) VALUES (?, ?, ?, ?)");
                $stmt->execute([$username, $email, $hashed_password, $folder]);
